...
export default function TopNavCapsule() {
  ...
  const [anomalyCount, setAnomalyCount] = useState(4);
  const [isDragging, setIsDragging]   = useState(false);

  useEffect(() => {
    getAnomalies().then(res => setAnomalyCount(res.data?.length || 4)).catch(() => {});
  }, []);

  // Determine active lens
  const isRoleLens   = location.pathname.startsWith('/roles') || location.pathname === '/org-chart';
  const isPeopleLens = location.pathname.startsWith('/people') || location.pathname.startsWith('/ticketing') || location.pathname.startsWith('/wellbeing');
  ...
  return (
    <div className="w-full px-6 pt-3.5 pb-1.5 flex-shrink-0 z-30">
      <header className="bg-white dark:bg-[#0A1224]/75 dark:backdrop-blur-2xl border border-slate-200/90 dark:border-white/10 shadow-md shadow-slate-200/60 dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.6)] rounded-2xl px-5 py-2 flex items-center justify-between gap-3 max-w-7xl mx-auto transition-all">
        ...
        {/* ═══════ LEFT: Brand ═══════ */}
        <Link to={persona === 'admin' ? '/' : `/people/${staffEmployeeId}`} className="flex items-center gap-2.5 group flex-shrink-0">
          <div className="w-7 h-7 bg-white dark:bg-slate-900/90 rounded-lg p-1 border border-slate-200 dark:border-cyan-500/30 shadow-xs dark:shadow-[0_0_12px_rgba(0,191,255,0.25)] flex items-center justify-center transition-all group-hover:scale-105">
          ...
          <span className="hidden sm:inline text-sm font-black text-slate-900 dark:text-white tracking-tight group-hover:text-setel-600 dark:group-hover:text-cyan-400 transition-colors">
            OrgLens
          </span>
        </Link>

        {/* ═══════ CENTER: Dual-Lens Toggle + Optional Timeline Scrubber ═══════ */}
        <div className="flex items-center gap-3 flex-1 justify-center min-w-0">
          {/* Dual-Lens Pill */}
          <div className="bg-slate-100 dark:bg-slate-900/80 p-0.5 rounded-xl flex items-center border border-slate-200 dark:border-white/10 shadow-inner flex-shrink-0">
            <button
              onClick={() => navigate('/org-chart')}
              className={`px-3 py-1.5 rounded-lg text-[11px] font-extrabold transition-all duration-150 ${
                isRoleLens 
            ...
            >Role Lens</button>
            <button
              onClick={() => navigate(persona === 'admin' ? '/people' : `/people/${staffEmployeeId}`)}
              className={`px-3 py-1.5 rounded-lg text-[11px] font-extrabold transition-all duration-150 ${
                isPeopleLens 
                  ? 'bg-setel-500 dark:bg-cyan-400 text-slate-950 shadow-sm dark:shadow-[0_0_15px_rgba(0,191,255,0.4)]' 
                  : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
          ...
          {/* ──── Timeline Player & Scrubber: Render ONLY on Interactive Org Chart (/org-chart) ──── */}
        ...
        {/* ═══════ RIGHT: Scenario badge, anomalies, prominent theme segmented pill, profile ═══════ */}
        <div className="flex items-center gap-2.5 flex-shrink-0">
          ...
          {/* Anomaly Badge */}
          <Link to="/"
            className="flex items-center gap-1 text-[10px] font-extrabold text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 hover:bg-amber-100 dark:hover:bg-amber-900/50 border border-amber-200 dark:border-amber-500/30 px-2 py-1 rounded-xl shadow-xs transition-all">
            <ShieldAlert size={12} className="text-amber-600 dark:text-amber-400" />
            <span>{anomalyCount}</span>
          </Link>

          {/* Prominent Light / Dark Segmented Switcher */}
