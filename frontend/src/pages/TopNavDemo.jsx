  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {[
          { title: 'Total Employees', value: stats.total_employees, img: '/ai_icons/employees.jpg', glow: 'dark:shadow-[0_0_20px_rgba(0,191,255,0.15)]', border: 'border-setel-200 dark:border-cyan-500/50' },
          { title: 'Total Roles', value: stats.total_roles, img: '/ai_icons/roles.jpg', glow: 'dark:shadow-[0_0_20px_rgba(59,130,246,0.15)]', border: 'border-blue-200 dark:border-blue-500/50' },
          { title: 'Departments', value: stats.total_departments, img: '/ai_icons/departments.jpg', glow: 'dark:shadow-[0_0_20px_rgba(99,102,241,0.15)]', border: 'border-indigo-200 dark:border-indigo-500/50' },
          { title: 'Active Anomalies', value: stats.active_anomalies, img: '/ai_icons/anomalies.jpg', glow: 'dark:shadow-[0_0_20px_rgba(251,191,36,0.15)]', border: 'border-amber-200 dark:border-amber-500/50' }
        ].map((stat, i) => (
          <div key={i} className={`bg-white/90 dark:bg-[#0C1527]/70 backdrop-blur-xl p-5 rounded-3xl shadow-sm border border-slate-200/80 dark:border-white/10 ${stat.glow} flex items-center gap-4 transition-all duration-200 hover:scale-[1.02] hover:border-slate-300 dark:hover:border-cyan-400/40`}>
            <div className={`w-14 h-14 rounded-2xl flex-shrink-0 overflow-hidden border-2 ${stat.border} shadow-sm group-hover:scale-105 transition-transform duration-300`}>
              <img src={stat.img} alt={stat.title} className="w-full h-full object-cover" />
            </div>
            <div>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{stat.title}</p>
              <h3 className="text-2xl font-black text-slate-800 dark:text-slate-50 mt-0.5">{stat.value || 0}</h3>
            </div>
          </div>
        ))}
      </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                ...
                { title: 'Role History & Relevancy', path: '/roles' },
